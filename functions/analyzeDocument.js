  ...
  async (request) => {
    ...
    try {
      ...
      if (!procId || procId === "not_configured") {
        console.error(
          "Document AI not configured - PROCESSOR_ID secret missing"
        );
      ...
      if (error.code === 3 || error.message.includes("INVALID_ARGUMENT")) {
        ...
        console.error(
          "5. Region mismatch - processor might not be in EU region"
        );
