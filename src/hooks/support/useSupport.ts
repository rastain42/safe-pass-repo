...
import { submitContactRequest, submitPhoneChangeRequest } from '@/services/support/support.service';
import { validateEmail, validatePhoneNumber } from '@/utils/validators';

// Type pour gérer les différentes sections
export type SupportSection = 'menu' | 'contact' | 'phoneChange' | 'passwordReset';
...
export function useSupport() {
  ...
  const handleSendContactRequest = async () => {
    ...
    try {
      ...
      await submitContactRequest('User', email, message);
      setSent(true);
    } catch (error) {
  ...
  const handlePhoneChangeRequest = async () => {
    ...
    try {
      ...
      await submitPhoneChangeRequest(email, oldPhoneNumber, newPhoneNumber);
      setSent(true);
    } catch (error) {
      console.error("Erreur lors de l'envoi de la demande de changement de numéro", error);
    }
  };
